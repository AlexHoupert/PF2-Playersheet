...
test('player page cutover uses registry renderer instead of header mode switch', () => {
    ...
    assert.match(carouselSource, /watchDrag: true/);
    assert.match(carouselSource, /player-tab-reorder-panel/);
    assert.doesNotMatch(carouselSource, /Reorder\.|framer-motion|trackComponent/);
    assert.match(carouselSource, /REORDER_HOLD_MS = 2000/);
...
test('creature presentation uses shared reveal constants and encounter actor sheets use real callbacks', () => {
    ...
    const loreSource = readSource('src/player/lore/PlayerBestiaryLibrary.jsx');
