...
// ============================================================================
/**
 * Parse @Localize references into actual text
 */
const LOCALIZE_GLOSSARY = {
    'PF2E.NPC.Abilities.Glossary.NegativeHealing': 'A creature with void healing draws health from void energy rather than vitality energy. It is damaged by vitality damage and is not healed by vitality healing effects.',
    'PF2E.NPC.Abilities.Glossary.Grab': 'The monster attempts to Grapple the creature using the body part it attacked with. This attempt neither applies nor counts toward the creature\'s multiple attack penalty.',
    'PF2E.NPC.Abilities.Glossary.Slow': 'A slow creature regains only 2 actions at the start of its turn. It can\'t use reactions.',
};

/**
 * Parse ability description text, replacing @Localize, @UUID, @Check, @Damage
 * @param {string} html - Raw description HTML
 * @returns {string} Parsed text (still HTML, but with refs resolved)
 */
export function parseAbilityText(html) {
    if (!html) return '';

    let text = html;

    // Replace @Localize[...] with glossary text
    text = text.replace(/@Localize\[([^\]]+)\]/g, (match, key) => {
        return LOCALIZE_GLOSSARY[key] || match;
    });

    // Replace @UUID[...]{Display} with just the display text (will be linked later)
    text = text.replace(/@UUID\[[^\]]+\]\{([^}]+)\}/g, '<span class="condition-link">$1</span>');

    // Replace @Check[...] with formatted check text
    text = text.replace(/@Check\[([^\]|]+)(?:\|([^\]]*))?\]/g, (match, type, options) => {
        const dcMatch = options?.match(/dc:(\d+)/);
        const dc = dcMatch ? dcMatch[1] : '?';
        return `<strong>${type.charAt(0).toUpperCase() + type.slice(1)} DC ${dc}</strong>`;
    });

    // Replace @Damage[...] with formatted damage
    text = text.replace(/@Damage\[([^\]]+)\]/g, (match, content) => {
        // content like "1d6[void]" or "2d4[bludgeoning]"
        return `<strong>${content.replace(/\[([^\]]+)\]/g, ' $1')}</strong>`;
    });

    // Replace @Template[shape|distance:X] with "Xft shape" (e.g., "15ft cone")
    text = text.replace(/@Template\[([^\]|]+)(?:\|([^\]]*))?\]/g, (match, shape, options) => {
        const distMatch = options?.match(/distance:(\d+)/);
        const distance = distMatch ? distMatch[1] : '';
        return `<strong>${distance ? distance + 'ft ' : ''}${shape}</strong>`;
    });

    // Replace inline damage like "2d4[fire|options:area-damage] damage" with "2d4 fire damage"
    text = text.replace(/(\d+d\d+)\[([^\]|]+)(?:\|[^\]]*)?]\s*damage/gi, (match, dice, damageType) => {
        return `<strong>${dice} ${damageType} damage</strong>`;
    });

    // Replace [[/gmr ROLL #Comment]]{Display} with just the display text
    text = text.replace(/\[\[\/gmr\s+[^\]]+\]\]\{([^}]+)\}/g, '<strong>$1</strong>');

    // Also handle [[/gmr ROLL]]{Display} without the # comment
    text = text.replace(/\[\[\/[a-z]+\s+[^\]]+\]\]\{([^}]+)\}/g, '<strong>$1</strong>');
