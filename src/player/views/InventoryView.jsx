...
const renderRow = (item, index, { enableEquipTap = false } = {}) => {
    ...
    // Fallback to custom DB items
    if (!fromIndex && db?.shop?.customItems && item.name) {
        const customItem = db.shop.customItems[item.name];
        if (customItem) {
            ...
            fromIndex = {
                ...
                type: customItem.type ? (customItem.type.charAt(0).toUpperCase() + customItem.type.slice(1)) : 'Item', // Cap for bucket matching
