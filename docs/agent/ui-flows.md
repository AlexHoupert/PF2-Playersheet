...
Campaign member settings own `skillSortMode`, `loopPages`, and `pageOrderByCategory`. The registry normalizes personal page order once and supplies the same ordered pages to the top carousel, category drawer, page carousel, defaults, and swipe selectors. Hidden pages such as the reserved Proficiencies page remain in the internal order contract but never appear in runtime navigation. Holding a tab still for two seconds opens a separate reorder panel; dragging and confirming there changes the order without mounting another transform or drag system inside Embla's track. Movement before activation remains a native Embla swipe, while a short press remains a normal selection. `Alt+ArrowLeft` and `Alt+ArrowRight` provide the keyboard equivalent. Dynamic tabs retain their stored slot while unavailable.
...
- `knowledge.notes` is the sortable/filterable overview for all notes owned by the assigned Actor. It reuses the contextual editor, resolves current visible targets first, and falls back to the note's safe `targetSnapshot` when a source becomes unavailable.
