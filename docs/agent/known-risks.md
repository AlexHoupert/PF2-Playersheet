...
- Scaly Skin, Quicksilver Mutagen, Bless, and Metal Carapace now use declarative catalog EffectDefinitions and the shared resolver. Runtime seed injection remains a compatibility bridge until the reviewed `backfill:catalog-effects` write is explicitly approved and verified.
...
- The catalog Effect backfill currently plans 27 writes and has not been run live. Keep dry-run, backup, target identity, and explicit `--write --confirm-write` gates intact.
