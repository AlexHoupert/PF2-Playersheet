...
- Scaly Skin, Quicksilver Mutagen, Bless, and Metal Carapace now use declarative catalog EffectDefinitions and the shared resolver. Their 27 global overrides were backfilled and read-only verified on 2026-07-17; runtime seed injection remains a defensive compatibility bridge for missing or imported data.
...
- Effect selector inheritance is centralized in `effectResolver`: concrete Saves, Skills, Attacks, Perception, AC, and DCs include their applicable `all.checks`/`all.dcs` and attribute selectors. New calculation paths must use the resolver rather than adding global modifiers in UI code.
- Standard Condition rules now expose a shared rule tree. `Prone` derives Off-Guard AC and a separate all-attack penalty; `Grabbed` derives Off-Guard AC and Immobilized speed `set 0`. Add future hierarchical rules in `conditionEffectRules.js`, not in Drawer components.
- Canonical attack Effect selectors are `attack.all`, `attack.strength`, `attack.dexterity`, `attack.melee`, and `attack.ranged`. Legacy `melee.attack` and `ranged.attack` remain read aliases only; new catalog definitions must use canonical selectors.
- The Actor Effects overview deliberately hides `ac.dex_cap` while retaining it in rules calculations. Persistent-damage averages are comparison-only and must never be presented as bonuses.
- Effect source Actor labels are intentionally conservative: only an external Actor with an explicit `cast`, `activate`, or `consume` trigger is shown. Manual Encounter Effects and self-sources remain unattributed.
...
- Firestore rules now also gate campaign catalog entries, immutable change events, Effect requests, player Lore contributions, campaign roles, and direct assigned-Actor Effect writes. Static contract tests cover the intended shape, but a real Firestore Rules Emulator run is still required before treating new rule changes as production-verified.
- Direct party Effect application relies on Firestore source attribution (`sourceActorId`, assigned member Actor, and `createdBy`). A client can still submit declarative modifier content; stronger trust boundaries would require server-side validation or a Cloud Function rather than Firestore Rules alone.
- The catalog Effect backfill completed with backup `catalog-effects-2026-07-17T12-24-48-968Z`; the post-write dry-run reported 0 pending writes and 27 existing definitions. Keep the backup and explicit confirmation gates for any future rerun or extension.
